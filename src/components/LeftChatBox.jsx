            ...
            <div className="h-[600px] w-[500px] overflow-auto p-2">
...

                {
                    ...
                    <div key={i} className="text-left my-1">
                        <div className={`flex items-start gap-2 ${c.sender === 'user-1' ? 'justify-end': 'justify-start'}`}>
                            <div className={`bg-slate-300 h-7 w-7 rounded-full relative ${c.sender === 'user-1' ? 'order-2' : 'order-1'}`}>
                                <div className="absolute h-2.5 w-2.5 rounded-full bg-green-500 bottom-0.5 right-0.5 border-2 border-white"></div>
                            </div>
                            <div className={`${c.sender === 'user-1' ? 'order-1' : 'order-2'}`}>
                                <div className={`border inline-block px-4 py-1.5 rounded-xl max-w-[350px] ${c.sender === 'user-1' ? 'bg-gradient-to-br from-pink-500 to bg-purple-500' : 'bg-gradient-to-br from-cyan-500 to bg-teal-400'}`}>
                                    <p className="text-base me-10 text-white">{c.message}</p>
                                    <p className="text-[10px] text-end text-white">{c.time}</p>
                                </div>
                            </div>
                        </div>  
                    </div>
